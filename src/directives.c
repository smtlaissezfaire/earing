...
#define ARG(n) (params->args[(n)])
#define VALUE(n,t) (t)(ARG(n)->value)
#define TYPE(n,t) (params->args[(n)]->id == TK_##t)
#define ARITY(d,n) assert(params->i == (n) && "Directive " # d " takes " # n " parameter(s).")
#define ARITY_MIN(d,n) assert(params->i >= (n) && "Directive " # d " takes at least " # n " parameter(s).")
...
{
    Library *lib = Naming_search(state->libraries, module->start, module->len);
    if(lib) {
...
void library_call(Module *state, Parameters *params)
{
    ...
    if(lib) {
        ...
        state->libraries = Naming_insert(state->libraries, lib->name, lib->len, lib);
    } else {
        exit(1);
    }
}

void library_destroy_cb(void *p)
{
    Library *lib = p;
...
void library_destroy(Module *state, Parameters *params)
{
    if(state->libraries) {
        Naming_destroy(state->libraries, (Naming_destroy_cb)library_destroy_cb);
...
void import(Module *state, Parameters *params)
{
    size_t length = 0;
    char *input = NULL;
    Module *target = NULL;
    ...
    if(Naming_search(state->imports, ARG(1)->start, ARG(1)->len)) {
    ...
    state->imports = Naming_insert(state->imports, ARG(1)->start, ARG(1)->len, target);
...
void Module_register_default_directives(Module *state)
{
    ...
    Module_register_directive(state, "library", strlen("library"), library_call, library_destroy);
}
