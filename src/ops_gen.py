...
function_decl = "int inst_%(op)s(Module *state, Token *type, Parameters *params);\n"

typed_function = """
int inst_%(op)s(Module *state, Token *type, Parameters *params) {
    if(params->i != %(pcount)s)
...
for op in final:
    op["args"] = ", ".join(["params->args[%d]->value" % x for x in range(0,op["pcount"])])
